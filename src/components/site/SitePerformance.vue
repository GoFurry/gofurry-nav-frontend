<template>
  <section class="">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <!-- 响应时间 -->
      <div
          class="rounded-xl shadow-sm p-5 transition-all duration-300 hover:shadow-md transform hover:scale-[1.03]"
          :class="responseColor"
      >
        ...
        <p class="text-2xl font-bold">{{ metrics.responseTime }}</p>
      ...
      >
        ...
        <p class="text-2xl font-bold">{{ metrics.statusCode }}</p>
      ...
      >
        ...
        <p class="text-2xl font-bold">{{ metrics.tlsVersion }}</p>
      ...
      >
        ...
        <p class="text-2xl font-bold">{{ metrics.certDaysLeft }}</p>
        ...
        <div class="flex items-center mb-2 md:mb-0">
          ...
          <div class="flex gap-2">
            <button
                ...
                :class="[
                'px-3 py-1 rounded-lg text-sm transition-all duration-300 ease-in-out transform hover:scale-105',
                sampleType === type
...
const getColor = (metric: string, value: any) => {
  switch (metric) {
    case 'responseTime': {
      ...
      if (time < 300) return 'border-l-4 border-green-500 bg-green-50'
      if (time < 800) return 'border-l-4 border-yellow-500 bg-yellow-50'
      return 'border-l-4 border-red-500 bg-red-50'
    }
    case 'statusCode': {
      if (value >= 200 && value < 300) return 'border-l-4 border-green-500 bg-green-50'
      if (value >= 300 && value < 400) return 'border-l-4 border-yellow-500 bg-yellow-50'
      return 'border-l-4 border-red-500 bg-red-50'
    }
    case 'tlsVersion': {
      if (value.includes('1.3')) return 'border-l-4 border-green-500 bg-green-50'
      if (value.includes('1.2')) return 'border-l-4 border-yellow-500 bg-yellow-50'
      return 'border-l-4 border-red-500 bg-red-50'
    }
    case 'certDaysLeft': {
      const days = parseInt(String(value).replace(/[^0-9]/g, ''), 10) || 0
      if (days > 90) return 'border-l-4 border-green-500 bg-green-50'
