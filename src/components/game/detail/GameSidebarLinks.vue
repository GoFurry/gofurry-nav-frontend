  ...
  <div class="bg-orange-50 rounded-2xl p-4 shadow text-sm space-y-3">
    <div class="flex justify-between gap-x-2">
      ...
      <button
          v-if="game?.appid"
          @click="goSteam"
          class="w-full py-2 rounded-lg bg-orange-400 text-white hover:bg-orange-300 transition"
      >
        {{t("game.detail.goToSteamPage")}}
      </button>
      <button
          class="w-full py-2 rounded-lg bg-orange-200 text-orange-800 hover:bg-orange-100 transition"
      >
...
const goSteam = () => {
  ...
  window.open(`https://store.steampowered.com/app/${props.game.appid}`, '_blank')
}
